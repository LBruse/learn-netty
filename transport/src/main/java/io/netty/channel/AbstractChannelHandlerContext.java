...
//        inbound标识(判断ChannelHandler instanceof InboundChannelHandler)
...
//        选择当前节点的后置节点进行异常传播
//        当传递到tail节点后,打印异常未处理信息,且释放ByteBuffer
...
//        找到下一个Inbound head,将事件继续往下传播
    ...
    static void invokeChannelRead(final AbstractChannelHandlerContext next, Object msg) {
        ...
        EventExecutor executor = next.executor();
//        当前执行线程是NioEventLoop,执行invokeChannelRead(Obj),
//        否则将任务丢到NioEventLoop的MpscQueue
        if (executor.inEventLoop()) {
    ...
    private void invokeChannelRead(Object msg) {
        if (invokeHandler()) {
            try {
//                将当前事件传播中的msg对象通过ctx(DefaultChannelPipeline).fireChannelRead(obj)进行传播
//                如果此时已经传播到了tail节点,会打印消息没有处理信息
...
//        调用ChannelOutboundHandler.write()进行事件传播
        try {
...
//        flush:true  表示write()带刷新
//        flush:false 仅仅把对象写到netty缓存,并没有实际把数据写到对应的service
...
//        findContextOutbound()与findContextInbound()相对
        AbstractChannelHandlerContext next = findContextOutbound();
        final Object m = pipeline.touch(msg, next);
        EventExecutor executor = next.executor();
//        判断执行线程是否NioEventLoop
        if (executor.inEventLoop()) {
//            是否flush校验
            if (flush) {
...
//            封装一个Task,把Task扔到NioEventLoop的MpscQueue
...
//        直接将ChannelHandler状态设置为REMOVE_COMPLETE
        handlerState = REMOVE_COMPLETE;
    }

    /**
     * 通过自旋和CAS操作,将ChannelHandler状态设置为ADD_COMPLETE
     */
