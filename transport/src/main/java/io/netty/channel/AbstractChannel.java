...
//            当前eventLoop绑定到客户端Channel
            AbstractChannel.this.eventLoop = eventLoop;

//            此时发起线程是服务端NioEventLoop,并非客户端NioEventLoop
            if (eventLoop.inEventLoop()) {
                register0(promise);
            } else {
                try {
//                    启动chooser选中的NioEventLoop
...
//                将NioSocketChannel注册到chooser.next()选中的NioEventLoop的Selector
                doRegister();
...
//                调用ChannelInitializer.handlerAdded()
...
//                新连接已经接入,且已经注册到selector,isActive()返回true
                if (isActive()) {
//                    是否第一次注册到NioEventLoop
