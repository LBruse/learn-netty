...
//    链表中第一个已经被flush掉的entry
    private Entry flushedEntry;
    // The Entry which is the first unflushed in the linked-list structure
//    链表中第一个没有被flush掉的entry
...
//            往tail指针追加一个entry,并移动tail指针
            Entry tail = tailEntry;
            tail.next = entry;
            tailEntry = entry;
        }
//        如果unflushed指针等于空[第一次添加]
...
//        获取第一个unflushed Entry[unflushed指针]
        Entry entry = unflushedEntry;
        if (entry != null) {
            if (flushedEntry == null) {
                // there is no flushedEntry yet, so start with the entry
//                将flushed指针指向第一个unflushed Entry[unflushed指针]
                flushedEntry = entry;
            }
            do {
//                当前可flush对象计数
...
//            将unflushed指针设为null,所有数据都可flush
            unflushedEntry = null;
        }
    }
...
//        TOTAL_PENDING_SIZE_UPDATER 当前缓冲区待写字节
//        累加当前待写字节
...
//            CAS操作
            if (UNWRITABLE_UPDATER.compareAndSet(this, oldValue, newValue)) {
                if (oldValue == 0 && newValue != 0) {
//                    传播ChannelWritabilityChanged事件,通知ChannelHandler当前不能再写入数据了,当前Channel为不可写状态
                    fireChannelWritabilityChanged(invokeLater);
                }
                break;
            }
