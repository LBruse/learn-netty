...
//        如果第一个任务截止时间<传进来的截止时间
        if (scheduledTask.deadlineNanos() <= nanoTime) {
//            把这个定时任务从定时任务队列中remove,并返回定时任务
            scheduledTaskQueue.remove();
            return scheduledTask;
        }
//        当前定时任务队列中所有任务的截止时间都还没到,返回null
        return null;
    }
...
//        把callable封装成ScheduledFutureTask
...
//        判断当前线程是否NioEventLoop
        if (inEventLoop()) {
//            是NioEventLoop,直接进行添加
            scheduledTaskQueue().add(task);
        } else {
//            不是NioEventLoop,使用线程提交任务的方式进行任务添加(线程安全)
