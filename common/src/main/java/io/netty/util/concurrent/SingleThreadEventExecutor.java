...
//        保存线程执行器,创建NioEventLoop时需要用到
        this.executor = ObjectUtil.checkNotNull(executor, "executor");
//        创建TaskQueue Mpsc Queue
...
//                如果定时任务添加到普通任务队列中失败,则将其重新添加到定时任务队列
                // No space left in the task queue add it back to the scheduledTaskQueue so we pick it up again.
                scheduledTaskQueue().add((ScheduledFutureTask<?>) scheduledTask);
                return false;
            }
//            继续拉取截止时间为nanoTime的任务
...
//        从定时任务队列里聚合任务
//        (将已到截止时间的定时任务从定时任务队列中移除,并添加到普通任务队列)
        fetchFromScheduledTaskQueue();
//        从普通任务队列中获取一个任务
...
//            当当前执行完毕任务数==64
            if ((runTasks & 0x3F) == 0) {
//                计算当前时间(nanoTime()是一个相对比较耗时的操作)
                lastExecutionTime = ScheduledFutureTask.nanoTime();
//                如果当前时间已经超过截止时间,不再继续执行任务
...
//        判断当前线程是否是NioEventLoop线程
        boolean inEventLoop = inEventLoop();
//        是NioEventLoop,把task添加到taskQueue中,串行执行
        if (inEventLoop) {
            addTask(task);
        } else {
//            创建NioEventLoop(FastThreadLocalThread)线程
            startThread();
//            把task添加到新创建的NioEventLoop的taskQueue
            addTask(task);
...
//                实际启动线程方法
    ...
    private void doStartThread() {
        assert thread == null;
//        executor --> ThreadPreTaskExecutor
//        创建FastThreadLocalThread并启动
        executor.execute(new Runnable() {
            @Override
            public void run() {
//                将新创建的FastThreadLocalThread保存到NioEventLoop
                ...
                try {
//                    调用NioEventLoop.run()进行启动(此时NioEventLoop线程已被创建且已start)
